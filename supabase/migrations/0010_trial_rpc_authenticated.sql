-- Allow the signed-in user to activate their own trial (no JWT service_role / sb_secret).
-- service_role can still call the function for admin/ops.
...
grant execute on function public.activate_free_trial(uuid, integer) to authenticated;
